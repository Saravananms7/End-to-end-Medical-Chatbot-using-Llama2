...
![MedBot AI Web Interface Preview](static/preview.png)
...
```
[Medical PDFs in data/] ──> (DirectoryLoader) ──> (RecursiveCharacterSplitter)
                                                                 │
[Pinecone Vector Index] <── (Upsert Vectors) <── (sentence-transformers Embeddings)
         │
         ▼
[User Query] ──> (HuggingFace Embeddings) ──> (K-Nearest Neighbors Search)
                                                         │
                                                         ▼
[Context + Question] ──> (RetrievalQA Prompt) ──> [Llama 2 Local LLM] ──> [AJAX Chat UI]
```
